...
namespace Test1
{
    public static class Program
    {
        static void Main(string[] args)
        {
            ...
            //WL(算单(已知牌[3], 空碰牌数组[3], 1));
            //WL(算单(已知牌[3], 空碰牌数组[3], 2));
            //WL(算单(已知牌[3], 空碰牌数组[3], 3));
            //WL(算单(已知牌[3], 空碰牌数组[3], 7));
        ...
        /// 用于传递空参数
        /// </summary>
        public static 碰型[][] 空碰牌数组 = new 碰型[4][] { new 碰型[] { }, new 碰型[10], new 碰型[10], new 碰型[10] };
        /// <summary>
        /// 用于传递空参数
        /// </summary>
        public static 杠型[][] 空杠牌数组 = new 杠型[4][] { new 杠型[] { }, new 杠型[10], new 杠型[10], new 杠型[10] };
        ...
        /// 判断玩家打出哪一张手牌比较好
        /// </summary>
        /// <param name="已知牌">当前玩家知道的明牌</param>
        /// <param name="碰牌">当前玩家手里的碰牌</param>
        /// <param name="杠牌">当前玩家手里的杠牌</param>
        /// <param name="手牌">当前玩家手里的自由牌</param>
        /// <param name="所有牌">当前玩家手里的牌</param>
        /// <param name="定张花"></param>
        /// <returns></returns>
        ...
        /// </summary>
        public static 牌? 判断碰(int[][] 已知牌, 碰型[][] 碰牌, 杠型[][] 杠牌, int[][] 手牌, int[][] 所有牌, 定张 定张花, 牌 目标牌)
        {
            // 如果是定张，返回不可以碰
        ...
        /// 打，杠，胡，碰，放弃  N选1
        /// </summary>
        public static 选项 行为判断(int[][] 已知牌, 碰型[][] 碰牌, 杠型[][] 杠牌, int[][] 手牌, int[][] 所有牌, 定张 定张花, int 剩余玩家人数, params 选项[] 可选项)
        {
            // 思路：foreach 可选项, 分别打分，找出最高分
            // 根据  可选项 是否包含  行为：打  判断 ： true 当前是 轮到自己（打，杠，胡） ，false 其他玩家打牌出来（碰，杠，胡，放弃）
            // 其中：暗杠，弯杠 可能存在多组
            ...
            // 初步实现：有胡必胡
            // todo: 下一步实现：给胡牌打分，判断要不要胡（比如手上有几个刻子，有人点屁胡炮）
            var hu = 可选项.FirstOrDefault(a => a.行为 == 行为.胡);
            if (hu != null) return hu;

            // 创建打分存储数组
            var max = 0;
            选项 result = null;
            foreach (var c in 可选项)
            {
                var f = 0;
                var p = c.目标牌;
                var ph = p.花;
                    ...
                    case 行为.碰:
                        {
                            ...
                            // 找出碰后打哪一张合适
                            c.目标牌 = 选弃张(已知牌, 碰牌, 杠牌, 手牌, 所有牌, 定张花);
                            var qh = c.目标牌.花;
                            var qd = c.目标牌.点;
        ...
        // 最终调用体：
        // 对于庄家打牌前，或刚摸起一张时： 判断摸到牌之后的抉择，返回 行为（暗杠，弯杠，打），目标牌（杠牌，舍牌）  判断杠打
        // 对于别人打出一张：判断对于目标牌的抉择，返回 行为（放弃，碰，杠），目标牌（碰后需要打的）  判断碰杠
