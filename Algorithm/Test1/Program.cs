...
namespace Test1
{
    public static class Program
    {
        ...
        static int 算牌(int[][] 已知牌, 碰型[][] 碰牌, 杠型[][] 杠牌, int[][] 手牌, int[][] 所有牌, 定张 定张花)
        {
            ...
            if (所有牌[1][0] > 0 && (byte)定张花 != 1) p += 算花(已知牌[1], 碰牌[1], 杠牌[1], 手牌[1]);
            if (所有牌[2][0] > 0 && (byte)定张花 != 2) p += 算花(已知牌[2], 碰牌[2], 杠牌[2], 手牌[2]);
            if (所有牌[3][0] > 0 && (byte)定张花 != 3) p += 算花(已知牌[3], 碰牌[3], 杠牌[3], 手牌[3]);
        ...
        static 牌? 判断碰(int[][] 已知牌, 碰型[][] 碰牌, 杠型[][] 杠牌, int[][] 手牌, int[][] 所有牌, 定张 定张花, 牌 目标牌)
        {
            ...
            // todo: 判断有几种花色的牌，如果是三种花色，碰后要打的牌将受到限制，下面的算法不合适
            // todo: 三花，碰之后依次打掉另两门的牌算分 找出最高值
            // todo: 已有的 碰，杠　要扫。如果已经存在两种花色，则　即便未启用定张，也认为　定张花　为缺失的那门牌

            // todo: 算法思路：对于三种花色来说，碰之前的分 = 两种最高分的花 加起来，碰之后的分同理。定张花色不算分

            byte h = (byte)定张花;

            if (h == 0)
            {
                // 判断已有碰杠牌的情况，从而决定正确的花色取舍
                bool b1 = 碰牌[1][0] > 0 | 杠牌[1][0] > 0,
                    b2 = 碰牌[2][0] > 0 | 杠牌[2][0] > 0, 
                    b3 = 碰牌[3][0] > 0 | 杠牌[3][0] > 0;
                var len = (b1 ? 1 : 0) + (b2 ? 1 : 0) + (b3 ? 1 : 0);       // 取有多少个花色
                if (len == 0)           // 没有碰杠
                {
                    // 考虑：碰前无定张，碰后必须舍弃一门，这个前后的分值计算方式不同
                }
                else if (len == 1)      // 有一种花色的碰杠
                {
                    // 考虑：判断是否与当前目标牌花色重合，如果不重合，则属须考虑：碰前无定张（须舍一门），碰后有定张，舍弃定张门，这个前后的分值计算方式不同
                }
                else if (len == 2)      // 两种花色的碰杠
                {
                    // 推算出缺失花色为定张花色
                }
            }
            ...
            // 找出分最高的 碰后弃张
            var hs = 布尔转索引s(所有牌[1][0] > 0, 所有牌[2][0] > 0, 所有牌[3][0] > 0);
